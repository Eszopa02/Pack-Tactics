...
namespace FronkonGames.LUTs.Cyberpunk
{
  ...
  public abstract partial class Inspector
  {
    ...
    public static Color ColorField(string label, string tooltip, Color value, Color resetValue) =>
      ColorField(label, tooltip, value, false, false, resetValue);

    /// <summary> Color field with reset. </summary>
    public static Color ColorField(string label, string tooltip, Color value, bool showAlpha, bool hdr, Color resetValue)
    {
      EditorGUILayout.BeginHorizontal();
      {
        value = EditorGUILayout.ColorField(new GUIContent(label, tooltip), value, false, showAlpha, hdr);
    ...
    /// <summary> Toggle foldout. </summary>
    public static bool ToggleFoldout(string label, bool value, ref bool display)
    {
      display = GetFoldoutDisplay(label);

      Rect rect = GUILayoutUtility.GetRect(16.0f, 22.0f, Styles.HeaderStyle);
      GUI.Box(rect, label, Styles.HeaderStyle);

      Rect toggleRect = new(rect.x + 4.0f, rect.y + 4.0f, 13.0f, 13.0f);
      if (Event.current.type == EventType.Repaint)
        Styles.CheckboxStyle.Draw(toggleRect, false, false, value, false);
