...
#if UNITY_EDITOR
using UnityEditor;
[CustomEditor(typeof(CyberpunkDemo))]
public class CyberpunkDemoWarning : Editor
{
...
public class CyberpunkDemo : MonoBehaviour
{
  ...
  private void ResetEffect()
  {
    ...
    if (profiles.Length > 0)
      settings.profile = profiles[profileIndex];
  }
  ...
  private void Awake()
  {
    ...
    this.enabled = Cyberpunk.IsInRenderFeatures();
  }

  private void Start()
  {
    settings = Cyberpunk.Instance.settings;
    ResetEffect();
  }
  ...
  private void OnGUI()
  {
    ...
    GUILayout.BeginHorizontal();
    {
      GUILayout.BeginVertical("box", GUILayout.Width(450.0f), GUILayout.Height(Screen.height));
      {
        ...
        if (profiles.Length > 0)
        {
          profileIndex = SliderField("Profile", profileIndex, 0, profiles.Length - 1);
          settings.profile = profiles[profileIndex];
        }
